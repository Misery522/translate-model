...
impl BridgeError {
    pub fn invalid() -> Self {
        Self::new("INVALID_REQUEST", "请求参数不符合桌面安全约束。", false, 0)
    }
    pub fn forbidden() -> Self {
        Self::new("FORBIDDEN", "当前窗口无权调用此操作。", false, 0)
    }
    ...
    fn invalid_response() -> Self {
        Self::new("INVALID_RESPONSE", "服务返回了无法识别的数据。", false, 0)
    }
    fn internal() -> Self {
        Self::new(
            "HOST_UNAVAILABLE",
    ...
    fn unpaired() -> Self {
        Self::new("PAIRING_REQUIRED", "请先使用电脑端配对码连接。", false, 401)
    }
    fn network() -> Self {
        Self::new(
            "NETWORK_UNCONFIRMED",
            "连接未完成；请求可能已送达，请先查询或停止任务。",
            true,
            0,
        )
    }
...
impl TranslationInput {
    fn validate(&self) -> Result<(), BridgeError> {
        ...
        if self.text.trim().is_empty()
            ...
            .contains(&self.domain.as_str())
            || !["auto", "translate", "annotate_code", "annotate_special"]
                .contains(&self.task_mode.as_str())
        {
    ...
    fn plan(&self) -> Result<RequestPlan, BridgeError> {
        let (method, suffix, body, auth) = match self {
            ...
            Self::DeleteSession { session_id } => {
                (Method::DELETE, format!("sessions/{session_id}"), None, true)
            }
            Self::Translate {
                session_id,
                client_request_id,
            ...
            Self::Cancel { job_id } => (
                Method::POST,
                format!("translations/{job_id}/cancel"),
                Some(json!({})),
                true,
            ),
...
pub fn validate_origin(raw: &str) -> Result<Url, BridgeError> {
    let invalid = || {
        BridgeError::new(
            "INVALID_ORIGIN",
            "仅支持 HTTPS 源地址，或带端口的本机 HTTP 地址；请勿包含路径或凭据。",
            false,
            0,
        )
    };
...
impl Bridge {
    ...
    pub async fn request(&self, request: ApiRequest) -> Result<ApiResponse, BridgeError> {
        ...
        let (origin, token, generation) = {
            let mut state = self.lock()?;
            let origin = state.origin.clone().ok_or_else(|| {
                BridgeError::new(
        ...
        let mut outgoing = self
            .client
            .request(plan.method, endpoint)
            .header(header::ACCEPT, "application/json");
        if logout {
            outgoing = outgoing.timeout(Duration::from_secs(3));
        }
        if plan.auth {
            let token = token.as_ref().ok_or_else(BridgeError::unpaired)?;
            let value = Zeroizing::new(format!("Bearer {}", token.as_str()));
            let mut authorization = header::HeaderValue::from_str(&value)
                .map_err(|_| BridgeError::invalid_response())?;
...
fn extract_pairing(value: &mut Value) -> Result<(Zeroizing<String>, Value), BridgeError> {
    ...
    let secret = value
        .as_object_mut()
        .and_then(|o| o.remove("access_token"))
        .ok_or_else(BridgeError::invalid_response)?;
