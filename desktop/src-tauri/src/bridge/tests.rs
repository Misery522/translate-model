...
fn rejects_credentials_and_non_origins() {
    ...
    for input in [
        credential.as_str(),
        "https://translate.example/api/v1",
        "https://translate.example/a/..",
        "https://translate.example//",
        "https://translate.example?x=1",
        "https://translate.example#x",
        "https://translate.example/%2e",
        "https://translate.example\\x",
        " https://translate.example",
        "https://translate.example\n",
    ] {
...
fn canonicalizes_https_default_port() {
    assert_eq!(
        validate_origin("https://translate.example:443/")
            .unwrap()
            .as_str(),
        "https://translate.example/"
    );
...
fn rejects_path_injection_in_job_and_session_ids() {
    for value in [
        json!({"operation":"job","job_id":"../../admin"}),
        json!({"operation":"delete_session","session_id":"x?y=z"}),
    ] {
        assert!(serde_json::from_value::<ApiRequest>(value).is_err());
    }
}
...
fn pairing_forces_bearer_and_never_sends_authorization() {
    let request = ApiRequest::Pair {
        code: "A".repeat(12),
        device_name: "译境桌面".into(),
    };
    ...
    assert!(serde_json::from_value::<ApiRequest>(
        json!({"operation":"pair","code":"A".repeat(12),"device_name":"pet","auth_mode":"cookie"})
    )
    .is_err());
}
...
fn rejects_bad_pairing_inputs() {
    for code in [
        ...
        assert!(ApiRequest::Pair {
            code,
            device_name: "pet".into()
        }
        .plan()
        .is_err());
    }
...
fn all_protected_operations_require_token() {
    ...
    for request in [
        ...
        ApiRequest::DeleteSession { session_id: id },
        ApiRequest::Job { job_id: id },
        ApiRequest::Cancel { job_id: id },
        ApiRequest::Glossary,
    ] {
...
fn translation_is_typed_bounded_and_forwarded_without_execution() {
    ...
    for (key, value) in [
        ("text", "a".repeat(4001)),
        ("text", "  ".into()),
        ("target_language", "unknown".into()),
        ("source_language", "unknown".into()),
        ("style", "execute".into()),
        ("domain", "shell".into()),
        ("task_mode", "command".into()),
    ] {
        let mut body = translation();
        body[key] = Value::String(value);
        assert!(serde_json::from_value::<TranslationInput>(body)
            .unwrap()
            .validate()
            .is_err());
    }
...
fn reconfigure_clears_token_and_invalidates_inflight_requests() {
    ...
    assert_eq!(
        bridge
            .lock()
            .unwrap()
            .check_generation(old.generation)
            .unwrap_err()
            .code,
        "STALE_OPERATION"
    );
...
fn rejects_cookie_and_malformed_pairing_responses() {
    for (key, value) in [
        ("auth_mode", json!("cookie")),
        ("csrf_token", json!("not-null")),
        ("access_token", json!("short")),
...
fn http_errors_are_fixed_and_redirects_are_rejected() {
    for status in [301, 302, 307, 308] {
        assert_eq!(BridgeError::http(status).code, "REDIRECT_BLOCKED");
    }
...
fn interrupted_202_body_preserves_unconfirmed_task_semantics() {
    ...
    let error = tauri::async_runtime::block_on(bridge.request(request))
        .err()
        .unwrap();
...
fn interrupted_chunked_body_is_a_network_failure_not_invalid_json() {
    ...
    let bridge = Bridge::new().unwrap();
    bridge.configure(&origin, false).unwrap();
    let error = tauri::async_runtime::block_on(bridge.request(ApiRequest::Health))
        .err()
        .unwrap();
    assert_eq!(error.code, "NETWORK_UNCONFIRMED");
    assert_eq!(server.join().unwrap().len(), 1);
...
fn pairing_uses_native_bearer_without_cookies_or_frontend_token() {
    ...
    let (origin, server) = serve(vec![
        json_response(
            201,
            pairing_reply(),
            "Set-Cookie: example-session=synthetic; Path=/\r\n",
        ),
...
fn logout_can_retry_revocation_but_cannot_translate_after_failure() {
    ...
    bridge.lock().unwrap().token = Some(Zeroizing::new("x".repeat(43)));
    assert_eq!(
        tauri::async_runtime::block_on(bridge.request(ApiRequest::Logout))
            .err()
            .unwrap()
            .code,
        "SERVICE_UNAVAILABLE"
    ...
    assert!(!bridge.status(false).unwrap().paired);
    assert_eq!(
        tauri::async_runtime::block_on(bridge.request(ApiRequest::CreateSession))
            .err()
            .unwrap()
            .code,
        "PAIRING_REQUIRED"
...
fn logout_disconnected_response_does_not_fabricate_success() {
    ...
    assert!(bridge.lock().unwrap().pending_logout.is_some());
    assert_eq!(
        tauri::async_runtime::block_on(bridge.request(ApiRequest::Auth))
            .err()
            .unwrap()
            .code,
        "PAIRING_REQUIRED"
...
fn logout_has_a_short_deadline_and_retains_only_retry_capability() {
    ...
    let error = tauri::async_runtime::block_on(bridge.request(ApiRequest::Logout))
        .err()
        .unwrap();
    assert_eq!(error.code, "NETWORK_UNCONFIRMED");
