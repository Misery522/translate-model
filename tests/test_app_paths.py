...
from yijing import paths as app_paths
...
def test_source_checkout_keeps_local_glossary(tmp_path, monkeypatch):
    ...
    assert app_paths.data_directory(tmp_path / "yijing" / "paths.py") == tmp_path / "data"
...
def test_wheel_uses_user_data_not_site_packages(tmp_path, monkeypatch):
    ...
    assert app_paths.data_directory(Path("site-packages/yijing/paths.py")) == user_data
